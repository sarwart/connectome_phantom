function final_streamlines=BDS_det(atlas_base,conn_map,map,stride_size,seeding,theta_thresh,connection,N_core)
                ...
                %angle calculation w.r.t current vector
                 ...
                 %with current orientation
                        ...
                        if min_angle(1)<=theta_thresh
                            ...
                            st=[st; new_block];
                            org_st=[org_st; map(new_block,temp_conn(angle_index(1)))];
           ...
           if ~(ix==new_ix && iy==new_iy && iz==new_iz)  
                ...
                if ~isempty(find(conn_map(new_block,:),1))
                    ...
                    if min_angle(1)<=theta_thresh
                        ...
                        origin_vector=temp_vector(angle_index(1),:);
                        st=[new_block; st];
                        org_st=[map(new_block,temp_conn(angle_index(1)));org_st];  
                    else
                        found=1;
